...
public partial class CommandLineArgsGenerator
{
    ...
    {
        // Build the bash script as a string
        var scriptParts = new List<string>
        {
            "# Bash completion script for \" + appName + @\"",
            "# Generated by TeCLI",
            "",
        ...
        scriptParts.Add("}");
        scriptParts.Add("");
        scriptParts.Add("complete -F _\" + appName + @\"_completions \" + appName + @\"");
    ...
    private MethodDeclarationSyntax GenerateZshCompletionMethodSyntax(List<CommandSourceInfo> commandHierarchies, GlobalOptionsSourceInfo? globalOptions)
    {
        ...
        {
            "#compdef \" + appName + @\"",
            "# Zsh completion script for \" + appName + @\"",
            "# Generated by TeCLI",
            "",
            "_\" + appName + @\"() {",
            "    local line state",
    ...
    private MethodDeclarationSyntax GeneratePowerShellCompletionMethodSyntax(List<CommandSourceInfo> commandHierarchies, GlobalOptionsSourceInfo? globalOptions)
    {
        var scriptParts = new List<string>
        {
            "# PowerShell completion script for \" + appName + @\"",
            "# Generated by TeCLI",
            "",
    ...
    private MethodDeclarationSyntax GenerateFishCompletionMethodSyntax(List<CommandSourceInfo> commandHierarchies, GlobalOptionsSourceInfo? globalOptions)
    {
        var scriptParts = new List<string>
        {
            "# Fish completion script for \" + appName + @\"",
            "# Generated by TeCLI",
            "",
            "# Global options",
            "complete -c \" + appName + @\" -s h -l help -d 'Show help information'",
            "complete -c \" + appName + @\" -l version -d 'Show version information'"
        };
        ...
        if (globalOptions != null && globalOptions.Options.Count > 0)
        {
            ...
            foreach (var option in globalOptions.Options)
            {
                ...
                scriptParts.Add($"complete -c \" + appName + @\"{shortPart} -l {option.Name} -d '{desc}'");
            }
        }
    ...
    private IEnumerable<string> GenerateFishCommandCompletionLines(CommandSourceInfo command)
    {
        var lines = new List<string>();
        var desc = command.Description ?? "";
        lines.Add($"complete -c \" + appName + @\" -n '__fish_use_subcommand' -a '{command.CommandName}' -d '{desc}'");

        // Subcommands
        foreach (var sub in command.Subcommands)
        {
            var subDesc = sub.Description ?? "";
            lines.Add($"complete -c \" + appName + @\" -n '__fish_seen_subcommand_from {command.CommandName}' -a '{sub.CommandName}' -d '{subDesc}'");
        }
        ...
        foreach (var action in command.Actions)
        {
            ...
            lines.Add($"complete -c \" + appName + @\" -n '__fish_seen_subcommand_from {command.CommandName}' -a '{action.ActionName}' -d '{actionDesc}'");
        }
                ...
                foreach (var param in action.Method.Parameters)
                {
                    ...
                    if (optionAttr != null)
                    {
                        ...
                        if (!string.IsNullOrEmpty(longName))
                        {
                            ...
                            lines.Add($"complete -c \" + appName + @\" -n '__fish_seen_subcommand_from {command.CommandName}'{shortPart} -l {longName}");
                        }
                    }
                }
            }
